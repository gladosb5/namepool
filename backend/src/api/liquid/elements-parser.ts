...
class ElementsParser {
  ...
  public async $parse() {
    ...
    try {
      this.isRunning = true;
      const result = await namecoinClient.getChainTips();
      ...
      for (let height = latestBlockHeight + 1; height <= tip; height++) {
        const blockHash: INamecoinApi.ChainTips = await namecoinClient.getBlockHash(height);
        const block: INamecoinApi.Block = await namecoinClient.getBlock(blockHash, 2);
  ...
  protected async $parseBlock(block: INamecoinApi.Block) {
  ...
  protected async $parseInputs(tx: INamecoinApi.Transaction, block: INamecoinApi.Block) {
  ...
  protected async $parsePegIn(input: INamecoinApi.Vin, vindex: number, txid: string, block: INamecoinApi.Block) {
    const namecoinTx: INamecoinApi.Transaction = await namecoinSecondClient.getRawTransaction(input.txid, true);
    const namecoinBlock: INamecoinApi.Block = await namecoinSecondClient.getBlock(namecoinTx.blockhash);
    const prevout = namecoinTx.vout[input.vout || 0];
    ...
    await this.$savePegToDatabase(block.height, block.time, prevout.value * 100000000, txid, vindex,
      outputAddress, namecoinTx.txid, prevout.n, namecoinBlock.height, namecoinBlock.time, 1);
  }

  protected async $parseOutputs(tx: INamecoinApi.Transaction, block: INamecoinApi.Block) {
  ...
  protected async $savePegToDatabase(height: number, blockTime: number, amount: number, txid: string,
    txindex: number, namecoinaddress: string, namecointxid: string, namecoinindex: number, namecoinblock: number, namecoinBlockTime: number, final_tx: number): Promise<void> {
    const query = `INSERT IGNORE INTO elements_pegs(
        block, datetime, amount, txid, txindex, namecoinaddress, namecointxid, namecoinindex, final_tx
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

    const params: (string | number)[] = [
      height, blockTime, amount, txid, txindex, namecoinaddress, namecointxid, namecoinindex, final_tx
    ];
    ...
    if (amount > 0) { // Peg-in
      ...
      await DB.query(`INSERT IGNORE INTO federation_addresses (namecoinaddress) VALUES (?)`, [namecoinaddress]);

      // Add the UTXO to the federation txos table
      const query_utxos = `INSERT IGNORE INTO federation_txos (txid, txindex, namecoinaddress, amount, blocknumber, blocktime, unspent, lastblockupdate, lasttimeupdate, timelock, expiredAt, emergencyKey, pegtxid, pegindex, pegblocktime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      const params_utxos: (string | number)[] = [namecointxid, namecoinindex, namecoinaddress, amount, namecoinblock, namecoinBlockTime, 1, namecoinblock - 1, 0, 4032, 0, 0, txid, txindex, blockTime];
      ...
      logger.debug(`Saved new Federation UTXO ${namecointxid}:${namecoinindex} belonging to ${namecoinaddress} to federation txos`);
  ...
  public async $updateFederationUtxos() {
    ...
    try {
      ...
      const namecoinBlocksToSync = await this.$getNamecoinBlockchainState();
      // If the namecoin blockchain is not synced yet, return
      if (namecoinBlocksToSync.namecoinHeaders > namecoinBlocksToSync.namecoinBlocks + 1) {
        logger.debug(`Namecoin client is not synced yet. ${namecoinBlocksToSync.namecoinHeaders - namecoinBlocksToSync.namecoinBlocks} blocks remaining to sync before the Federation audit process can start`);
        this.isUtxosUpdatingRunning = false;
        return;
      }
      ...
      while (auditProgress.lastBlockAudit <= auditProgress.confirmedTip) {
        ...
        if (auditProgress.confirmedTip - auditProgress.lastBlockAudit <= 150) { // If the audit status is not too far in the past, we can use gettxout (fast way)
          ...
          logger.debug(`Found ${utxos.length} Federation UTXOs and ${redeemAddresses.length} Peg-Out Addresses to scan in Namecoin block height #${auditProgress.lastBlockAudit} / #${auditProgress.confirmedTip}`);
          ...
          if (elapsedSeconds > 5) {
            ...
            logger.debug(`Scanning ${utxos.length} Federation UTXOs and ${redeemAddresses.length} Peg-Out Addresses at Namecoin block height #${auditProgress.lastBlockAudit} / #${auditProgress.confirmedTip} | ~${meanIndexingSpeed.toFixed(2)} blocks/sec | elapsed: ${(runningFor / 60).toFixed(0)} minutes | ETA: ${(eta / 60).toFixed(0)} minutes`);
            timer = Date.now() / 1000;
            indexedThisRun = 0;
          }
        }
        ...
        const blockHash: INamecoinApi.ChainTips = await namecoinSecondClient.getBlockHash(auditProgress.lastBlockAudit);
        const block: INamecoinApi.Block = await namecoinSecondClient.getBlock(blockHash, 2);
        await this.$parseNamecoinBlock(block, spentAsTip, unspentAsTip, auditProgress.confirmedTip, redeemAddresses);
  ...
  protected async $getFederationUtxosToScan(height: number) { 
    const query = `SELECT txid, txindex, namecoinaddress, amount, blocknumber, timelock, expiredAt FROM federation_txos WHERE lastblockupdate = ? AND unspent = 1`;
  ...
  protected async $getFederationUtxosToParse(utxos: any[]): Promise<any> {
    ...
    for (const utxo of utxos) {
      const result = await namecoinSecondClient.getTxOut(utxo.txid, utxo.txindex, false);
  ...
  protected async $parseNamecoinBlock(block: INamecoinApi.Block, spentAsTip: any[], unspentAsTip: any[], confirmedTip: number, redeemAddressesData: any[] = []) {
    const redeemAddresses: string[] = redeemAddressesData.map(redeemAddress => redeemAddress.namecoinaddress);
    for (const tx of block.tx) {
      ...
      for (const output of tx.vout) {
        if (output.scriptPubKey.address && federationChangeAddresses.includes(output.scriptPubKey.address)) {
          ...
          if (rows_check.length === 0) {
            ...
            const query_utxos = `INSERT INTO federation_txos (txid, txindex, namecoinaddress, amount, blocknumber, blocktime, unspent, lastblockupdate, lasttimeupdate, timelock, expiredAt, emergencyKey, pegtxid, pegindex, pegblocktime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
            ...
            spentAsTip.push({
              txid: tx.txid,
              txindex: output.n,
              namecoinaddress: output.scriptPubKey.address,
        ...
        if (mightRedeemInThisTx && output.scriptPubKey.address && redeemAddresses.includes(output.scriptPubKey.address)) {
          ...
          const matchingAddress: any[] = redeemAddressesData.filter(redeemAddress => redeemAddress.namecoinaddress === output.scriptPubKey.address && -redeemAddress.amount === Math.round(output.value * 100000000));
          if (matchingAddress.length > 0) {
            if (matchingAddress.length > 1) {
              ...
              logger.debug(`Found redeem txid ${tx.txid}:${output.n} to peg-out address ${matchingAddress[0].namecoinaddress}, amount ${matchingAddress[0].amount}, datetime ${matchingAddress[0].datetime}`);
            } else {
              logger.debug(`Found redeem txid ${tx.txid}:${output.n} to peg-out address ${matchingAddress[0].namecoinaddress}, amount ${matchingAddress[0].amount}`);
            }
            const query_add_redeem = `UPDATE elements_pegs SET namecointxid = ?, namecoinindex = ? WHERE namecoinaddress = ? AND amount = ? AND datetime = ?`;
            const params_add_redeem: (string | number)[] = [tx.txid, output.n, matchingAddress[0].namecoinaddress, matchingAddress[0].amount, matchingAddress[0].datetime];
  ...
  protected async $saveLastBlockAuditToDatabase(blockHeight: number) {
    const query = `UPDATE state SET number = ? WHERE name = 'last_namecoin_block_audit'`;
    await DB.query(query, [blockHeight]);
  }

  // Get the namecoin block where the audit process was last updated
  protected async $getAuditProgress(): Promise<any> {
    ...
    return {
      ...
      confirmedTip: namecoinBlocksToSync.namecoinBlocks - auditBlockOffsetWithTip,
    };
  }

  // Get the namecoin blocks remaining to be synced
  protected async $getNamecoinBlockchainState(): Promise<any> {
    const result = await namecoinSecondClient.getBlockchainInfo();
    return {
      namecoinBlocks: result.blocks,
      namecoinHeaders: result.headers,
    }
  }

  protected async $getLastBlockAudit(): Promise<number> {
    const query = `SELECT number FROM state WHERE name = 'last_namecoin_block_audit'`;
    const [rows] = await DB.query(query);
    return rows[0]['number'];
  }

  protected async $getRedeemAddressesToScan(): Promise<any[]> {
    const query = `SELECT datetime, amount, namecoinaddress FROM elements_pegs where amount < 0 AND namecoinaddress != '' AND namecointxid = '';`;
  ...
  public async $getAuditStatus(): Promise<any> {
    ...
    const namecoinBlocksToSync = await this.$getNamecoinBlockchainState();
    return {
      namecoinBlocks: namecoinBlocksToSync.namecoinBlocks,
      namecoinHeaders: namecoinBlocksToSync.namecoinHeaders,
      lastBlockAudit: lastBlockAudit,
      isAuditSynced: namecoinBlocksToSync.namecoinHeaders - namecoinBlocksToSync.namecoinBlocks <= 2 && namecoinBlocksToSync.namecoinBlocks - lastBlockAudit <= 3,
    };
  }
  ...
  public async $getCurrentLbtcSupply(): Promise<any> {
    ...
    const hash = await namecoinClient.getBlockHash(lastblockupdate);
    return {
  ...
  // Get the current reserves of the federation and the last Namecoin block it was updated
  public async $getCurrentFederationReserves(): Promise<any> {
    ...
    const lastblockaudit = await this.$getLastBlockAudit();
    const hash = await namecoinSecondClient.getBlockHash(lastblockaudit);
    return {
  ...
  public async $getFederationAddresses(): Promise<any> {
    const query = `SELECT namecoinaddress, SUM(amount) AS balance FROM federation_txos WHERE unspent = 1 AND expiredAt = 0 GROUP BY namecoinaddress ORDER BY balance DESC;`;
    const [rows] = await DB.query(query);
    return rows;
  }
  ...
  public async $getFederationUtxos(): Promise<any> {
    const query = `SELECT txid, txindex, namecoinaddress, amount, blocknumber, blocktime, pegtxid, pegindex, pegblocktime, timelock, expiredAt FROM federation_txos WHERE unspent = 1 AND expiredAt = 0 ORDER BY blocktime DESC;`;
    const [rows] = await DB.query(query);
    return rows;
  }
  ...
  public async $getExpiredUtxos(): Promise<any> {
    const query = `SELECT txid, txindex, namecoinaddress, amount, blocknumber, blocktime, pegtxid, pegindex, pegblocktime, timelock, expiredAt FROM federation_txos WHERE unspent = 1 AND expiredAt > 0 ORDER BY blocktime DESC;`;
    const [rows]: any[] = await DB.query(query);
    const feeRate = Math.round((await namecoinSecondClient.estimateSmartFee(1)).feerate * 100000000 / 1000);
    ...
    public async $getEmergencySpentUtxos(): Promise<any> {
      const query = `SELECT txid, txindex, namecoinaddress, amount, blocknumber, blocktime, pegtxid, pegindex, pegblocktime, timelock, expiredAt FROM federation_txos WHERE emergencyKey = 1 ORDER BY blocktime DESC;`;
      const [rows] = await DB.query(query);
      return rows;
    }
  ...
  public async $getFederationAddressesNumber(): Promise<any> {
    const query = `SELECT COUNT(DISTINCT namecoinaddress) AS address_count FROM federation_txos WHERE unspent = 1 AND expiredAt = 0;`;
    const [rows] = await DB.query(query);
    return rows[0];
  }
  ...
  public async $getPegsList(count: number = 0): Promise<any> {
    const query = `SELECT txid, txindex, amount, namecoinaddress, namecointxid, namecoinindex, datetime AS blocktime FROM elements_pegs ORDER BY block DESC LIMIT 15 OFFSET ?;`;
