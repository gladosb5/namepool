...
class DatabaseMigration {
  ...
  private async $createMissingTablesAndIndexes(databaseSchemaVersion: number) {
    ...
    const isNamecoin = ['mainnet', 'testnet', 'signet', 'testnet4'].includes(config.MEMPOOL.NETWORK);
    ...
    if (databaseSchemaVersion < 5 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.blocksTruncatedMessage);
    ...
    if (databaseSchemaVersion < 6 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.blocksTruncatedMessage);
      await this.$executeQuery('TRUNCATE blocks;');  // Need to re-index
    ...
    if (databaseSchemaVersion < 7 && isNamecoin === true) {
      await this.$executeQuery('DROP table IF EXISTS hashrates;');
      await this.$executeQuery(this.getCreateDailyStatsTableQuery(), await this.$checkIfTableExists('hashrates'));
      await this.updateToSchemaVersion(7);
    }

    if (databaseSchemaVersion < 8 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.blocksTruncatedMessage);
    ...
    if (databaseSchemaVersion < 9 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.hashratesTruncatedMessage);
      await this.$executeQuery('TRUNCATE hashrates;'); // Need to re-index
    ...
    if (databaseSchemaVersion < 10 && isNamecoin === true) {
      await this.$executeQuery('ALTER TABLE `blocks` ADD INDEX `blockTimestamp` (`blockTimestamp`)');
      await this.updateToSchemaVersion(10);
    }

    if (databaseSchemaVersion < 11 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.blocksTruncatedMessage);
      await this.$executeQuery('TRUNCATE blocks;'); // Need to re-index
    ...
    if (databaseSchemaVersion < 12 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 13 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 14 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.hashratesTruncatedMessage);
      await this.$executeQuery('TRUNCATE hashrates;'); // Need to re-index
    ...
    if (databaseSchemaVersion < 16 && isNamecoin === true) {
      this.uniqueLog(logger.notice, this.hashratesTruncatedMessage);
    ...
    if (databaseSchemaVersion < 17 && isNamecoin === true) {
      await this.$executeQuery('ALTER TABLE `pools` ADD `slug` CHAR(50) NULL');
      await this.updateToSchemaVersion(17);
    }

    if (databaseSchemaVersion < 18 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 20 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 22 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 24 && isNamecoin == true) {
      await this.$executeQuery('DROP TABLE IF EXISTS `blocks_audits`');
      await this.$executeQuery(this.getCreateBlocksAuditsTableQuery(), await this.$checkIfTableExists('blocks_audits'));
      await this.updateToSchemaVersion(24);
    }

    if (databaseSchemaVersion < 25 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 26 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 27 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 28 && isNamecoin === true) {
      if (config.LIGHTNING.ENABLED) {
    ...
    if (databaseSchemaVersion < 29 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 30 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 31 && isNamecoin == true) { // Link blocks to prices
    ...
    if (databaseSchemaVersion < 39 && isNamecoin === true) {
      await this.$executeQuery('ALTER TABLE `nodes` ADD alias_search TEXT NULL DEFAULT NULL AFTER `alias`');
      await this.$executeQuery('ALTER TABLE nodes ADD FULLTEXT(alias_search)');
      await this.updateToSchemaVersion(39);
    }

    if (databaseSchemaVersion < 40 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 48 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 49 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 57 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 62 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 67  && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 68 && config.MEMPOOL.NETWORK === "liquid") {
      ...
      await this.$executeQuery(`INSERT INTO federation_addresses (namecoinaddress) VALUES ('bc1qxvay4an52gcghxq5lavact7r6qe9l4laedsazz8fj2ee2cy47tlqff4aj4')`); // Federation change address
      await this.$executeQuery(`INSERT INTO federation_addresses (namecoinaddress) VALUES ('3EiAcrzq1cELXScc98KeCswGWZaPGceT1d')`); // Federation change address
      ...
      await this.$executeQuery(`INSERT INTO state VALUES('last_namecoin_block_audit', 0, NULL);`);
    ...
    if (databaseSchemaVersion < 71 && config.MEMPOOL.NETWORK === 'liquid') {
      ...
      await this.$executeQuery(`INSERT INTO federation_addresses (namecoinaddress) VALUES ('3EiAcrzq1cELXScc98KeCswGWZaPGceT1d')`); // Federation change address
      await this.$executeQuery(`UPDATE state SET number = 0 WHERE name = 'last_elements_block';`);
      await this.$executeQuery(`UPDATE state SET number = 0 WHERE name = 'last_namecoin_block_audit';`);
    ...
    if (databaseSchemaVersion < 72 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 76 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 81 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 82 && isNamecoin === true && config.MEMPOOL.NETWORK === 'mainnet') {
      await this.$fixBadV1AuditBlocks();
      await this.updateToSchemaVersion(82);
    }

    if (databaseSchemaVersion < 83 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 84 && isNamecoin === true) {
      await this.$executeQuery(`
        ALTER TABLE \`pools\`
          ADD INDEX \`slug\` (\`slug\`),
    ...
    if (databaseSchemaVersion < 85 && isNamecoin === true) {
      await this.$executeQuery(`
    ...
    if (databaseSchemaVersion < 86 && isNamecoin === true) {
      await this.$executeQuery(`
        ALTER TABLE \`nodes\`
    ...
    if (databaseSchemaVersion < 87 && isNamecoin === true) {
    ...
    if (databaseSchemaVersion < 92 && config.MEMPOOL.NETWORK === 'liquid') {
      await this.$executeQuery(`
        ALTER TABLE \`elements_pegs\`
          ...
          ADD INDEX \`namecoinaddress\` (\`namecoinaddress\`),
          ADD INDEX \`namecointxid\` (\`namecointxid\`)
      `);
    ...
    if (databaseSchemaVersion < 94) {
      ...
      if (!isNamecoin) {
        // Apply all the namecoin specific migrations to non-namecoin networks: liquid, liquidtestnet and testnet4 (!)
        // Version 5
        await this.$executeQuery('ALTER TABLE blocks ADD `reward` double unsigned NOT NULL DEFAULT "0"');
  ...
  private getCreateElementsTableQuery(): string {
    return `CREATE TABLE IF NOT EXISTS elements_pegs (
      ...
      namecoinaddress varchar(100) NOT NULL,
      namecointxid varchar(65) NOT NULL,
      namecoinindex int(11) NOT NULL,
  ...
  private getCreateFederationTxosTableQuery(): string {
    return `CREATE TABLE IF NOT EXISTS federation_txos (
      ...
      FOREIGN KEY (namecoinaddress) REFERENCES federation_addresses (namecoinaddress)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;`;
  }
