 ...
 */
class NamecoinBackendRoutes {
  private static tag = 'NamecoinBackendRoutes';

  public initRoutes(app: Application): void {
    app
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-mempool-entry', this.disableCache, this.$getMempoolEntry)
      .post(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'decode-raw-transaction', this.disableCache, this.$decodeRawTransaction)
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-raw-transaction', this.disableCache, this.$getRawTransaction)
      .post(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'send-raw-transaction', this.disableCache, this.$sendRawTransaction)
      .post(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'test-mempool-accept', this.disableCache, this.$testMempoolAccept)
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-mempool-ancestors', this.disableCache, this.$getMempoolAncestors)
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-block', this.disableCache, this.$getBlock)
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-block-hash', this.disableCache, this.$getBlockHash)
      .get(config.MEMPOOL.API_URL_PREFIX + 'internal/namecoin-core/' + 'get-block-count', this.disableCache, this.$getBlockCount)
    ;
  }

  /**
   * Disable caching for namecoin core routes
   *
   * @param req
   * @param res
   * @param next
  ...
  private static handleException(e: any, fnName: string, res: Response): void {
    if (typeof(e.code) === 'number') {
      ...
      logger.err(err, NamecoinBackendRoutes.tag);
  ...
  private async $getMempoolEntry(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const mempoolEntry = await namecoinClient.getMempoolEntry(txid);
      if (!mempoolEntry) {
        res.status(404).send();
        return;
      }
      ...
      NamecoinBackendRoutes.handleException(e, 'getMempoolEntry', res);
    }
  }
  ...
  private async $decodeRawTransaction(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const decodedTx = await namecoinClient.decodeRawTransaction(rawTx);
      if (!decodedTx) {
      ...
      NamecoinBackendRoutes.handleException(e, 'decodeRawTransaction', res);
    }
  }
  ...
  private async $getRawTransaction(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const decodedTx = await namecoinClient.getRawTransaction(txid, verboseNumber);
      if (!decodedTx) {
  ...
  private async $sendRawTransaction(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const txHex = await namecoinClient.sendRawTransaction(rawTx);
      if (!txHex) {
      ...
      NamecoinBackendRoutes.handleException(e, 'sendRawTransaction', res);
    }
  }
  ...
  private async $testMempoolAccept(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const txHex = await namecoinClient.testMempoolAccept(rawTxs);
      ...
      NamecoinBackendRoutes.handleException(e, 'testMempoolAccept', res);
    }
  }
  ...
  private async $getMempoolAncestors(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const ancestors = await namecoinClient.getMempoolAncestors(txid, verbose === 'true' ? true : false);
      if (!ancestors) {
      ...
      NamecoinBackendRoutes.handleException(e, 'getMempoolAncestors', res);
    }
  }
  ...
  private async $getBlock(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const block = await namecoinClient.getBlock(blockHash, verbosityNumber);
      if (!block) {
      ...
      NamecoinBackendRoutes.handleException(e, 'getBlock', res);
    }
  }
  ...
  private async $getBlockHash(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const block = await namecoinClient.getBlockHash(blockHeightNumber);
      if (!block) {
      ...
      NamecoinBackendRoutes.handleException(e, 'getBlockHash', res);
    }
  }

  private async $getBlockCount(req: Request, res: Response): Promise<void> {
    try {
      const count = await namecoinClient.getBlockCount();
      if (!count) {
      ...
      NamecoinBackendRoutes.handleException(e, 'getBlockCount', res);
    }
  }
}

export default new NamecoinBackendRoutes();
