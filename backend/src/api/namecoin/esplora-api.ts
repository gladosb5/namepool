...
import { SubmitPackageResult, TestMempoolAcceptResult } from './namecoin-api.interface';
import os from 'os';
import { namecoinCoreApi } from './namecoin-api-factory';
...
class ElectrsApi implements AbstractNamecoinApi {
  ...
  async $getTxIdsForBlock(hash: string, fallbackToCore = false): Promise<string[]> {
    try {
      ...
      if (fallbackToCore && isAxiosError(e) && e.response?.status === 404) {
        ...
        const coreBlock = await namecoinCoreApi.$getBlock(hash);
        if (coreBlock?.stale) {
          return namecoinCoreApi.$getTxIdsForBlock(hash);
        }
      }
      throw e;
    }
  ...
  async $getTxsForBlock(hash: string, fallbackToCore = false): Promise<IEsploraApi.Transaction[]> {
    try {
      ...
      if (fallbackToCore && isAxiosError(e) && e.response?.status === 404) {
        ...
        const coreBlock = await namecoinCoreApi.$getBlock(hash);
        if (coreBlock?.stale) {
          return namecoinCoreApi.$getTxsForBlock(hash);
        }
      }
      throw e;
    }
