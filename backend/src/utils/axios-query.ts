...
async function queryInternal(path: string, headers: Record<string, string>, throwOnFail: boolean): Promise<object | undefined> {
  const setDelay = (secs: number = 1): Promise<void> => new Promise(resolve => setTimeout(() => resolve(), secs * 1000));

  const axiosOptions: AxiosOptions = {
    headers: {
      'User-Agent': (config.MEMPOOL.USER_AGENT === 'mempool')
...
export async function query(path: string, throwOnFail: boolean = false): Promise<object | undefined> {
  return queryInternal(path, {}, throwOnFail);
}

export async function queryWithHeaders(path: string, headers: Record<string, string>, throwOnFail: boolean = false): Promise<object | undefined> {
  return queryInternal(path, headers, throwOnFail);
}
