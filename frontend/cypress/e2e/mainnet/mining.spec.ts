  ...
  if (baseModule === 'mempool') {
    ...
    describe('Miner page', () => {
      ...
      it('loads the mining pool page from the dashboard', () => {
        ...
        cy.get('[data-cy="namecoin-block-0-pool"]').click().then(() => {
          cy.waitForSkeletonGone();
          cy.wait('@pool');
      ...
      it('loads the mining pool page from the blocks page', () => {
        cy.visit('/mining');
        cy.waitForSkeletonGone();
        cy.get('[data-cy="namecoin-block-0-height"]').click().then(() => {
          cy.waitForSkeletonGone();
    ...
    describe('Mining Dashboard Landing page widgets', () => {
      ...
      it('shows the mempool blocks', () => {
        ...
        cy.get('[data-cy="mempool-block-0-total-fees"]').invoke('text').should('match', /(.*) NMC/);
      ...
      it('shows the mined blocks', () => {
        cy.get('[data-cy="namecoin-block-0-height"]').invoke('text').should('match', /(\d)/);
        cy.get('[data-cy="namecoin-block-0-fees"]').invoke('text').should('match', /~(.*) sat\/vB/);
        cy.get('[data-cy="namecoin-block-0-fee-span"]').invoke('text').should('match', /(.*) - (.*) sat\/vB/);
        cy.get('[data-cy="namecoin-block-0-total-fees"]').invoke('text').should('match', /(.*) NMC/);
        cy.get('[data-cy="namecoin-block-0-transactions"]').invoke('text').should('match', /(.*) transactions/);
        cy.get('[data-cy="namecoin-block-0-time"]').invoke('text').should('match', /((.*) ago|Just now)/);
        cy.get('[data-cy="namecoin-block-0-pool"]').invoke('text').should('match', /(\w)/);
      });
