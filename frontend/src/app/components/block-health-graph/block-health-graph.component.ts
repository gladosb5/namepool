...
export class BlockHealthGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.namecoin.graphs.block-health:See Namecoin block health visualized over time. Block health is a measure of how many expected transactions were included in an actual mined block. Expected transactions are determined using Namepool's re-implementation of Namecoin Core's transaction selection algorithm.`);
