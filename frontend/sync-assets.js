...
const syncUpstreamPromo = parseInt(process.env.SYNC_UPSTREAM_PROMO) === 1;
...
if (syncUpstreamPromo) console.log(`${LOG_TAG} SYNC_UPSTREAM_PROMO is set, syncing upstream promo video assets`);
...
(async () => {
  try {
    ...
    if (syncUpstreamPromo) {
      ...
      console.log(`${LOG_TAG} Skipping upstream promo video sync (set SYNC_UPSTREAM_PROMO=1 to enable).`);
    }
